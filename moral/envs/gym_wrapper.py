...
import envs.small_multitarget_v0
import envs.big_multitarget_v0
import envs.test_oneTarget_v0
...
class GymWrapper(gym.Env):
    ...
    def __init__(self, env_id):
        ...
        if env_id == 'randomized_v2':
            ...
            self.height = 16
            self.num_actions = 9
        elif env_id == 'small_multitarget_v0':
            self.layers = ('#', 'P', 'A', 'B', 'O')
            self.width = 7 
            self.height = 8 # 5x6 grid plus walls in each side
            self.num_actions = 4   
        elif env_id == 'test_oneTarget_v0':
            self.layers = ('#', 'P', 'A', 'B', 'O')
            self.width = 7 
            self.height = 8 # 4x4 grid plus walls in each side
            self.num_actions = 4   
        elif env_id == 'big_multitarget_v0':
            self.layers = ('#', 'P', 'A', 'B', 'O')
            self.width = 20 
            self.height = 14 # 5x6 grid plus walls in each side
            self.num_actions = 4   
    ...
    def reset(self):
        ...
        elif self.env_id == 'small_multitarget_v0':
            self.game = envs.small_multitarget_v0.make_game()
        elif self.env_id == 'big_multitarget_v0':
            self.game = envs.big_multitarget_v0.make_game()
        elif self.env_id == 'test_oneTarget_v0':
            self.game = envs.test_oneTarget_v0.make_game()
